...
export default function ProductBanner() {
    ...
    const scale1 = useTransform(scrollYProgress, [0, 0.25, 0.5], [1, 1, 1.15]);
    const filter1 = useTransform(scrollYProgress, [0.25, 0.5], ["blur(0px)", "blur(10px)"]);
    const rotateX1 = useTransform(scrollYProgress, [0.25, 0.5], [0, -8]);
    const y1 = useTransform(scrollYProgress, [0, 0.15, 0.25], [isMobile ? 80 : 120, 0, 0]);

    // Image 2: starts small and behind, scales up as it comes into view, then moves up to cover text
    const opacity2 = useTransform(scrollYProgress, [0.25, 0.5, 1], [0, 1, 1]);
    const scale2 = useTransform(scrollYProgress, [0.25, 0.5, 0.75, 1], [0.75, 1, 1.05, 1.1]);
    const filter2 = useTransform(scrollYProgress, [0.25, 0.5], ["blur(6px)", "blur(0px)"]);
    const y2 = useTransform(scrollYProgress, [0.25, 0.5, 1], [100, 0, isMobile ? -80 : -220]);
    // Image 3: appears above image 2 and slides left on scroll
    const opacity3 = useTransform(scrollYProgress, [0.35, 0.6, 1], [0, 1, 1]);
    const scale3 = useTransform(scrollYProgress, [0.35, 0.6, 1], [0.85, 1, 1.05]);
    const y3 = useTransform(scrollYProgress, [0.35, 0.6, 1], [40, -40, -60]);
    const x3 = useTransform(scrollYProgress, [0.6, 0.85, 1], [0, isMobile ? -80 : -160, isMobile ? -140 : -260]);
                            ...
                            >
                                ...
                                <div className="absolute inset-0 flex items-center justify-center">
                                    <motion.img
                                        ...
                                        className="border-4 border-[#90909075] rounded-3xl max-w-full max-h-full w-auto h-auto object-contain drop-shadow-2xl will-change-transform origin-center"
                                    />
                                </div>

                                {/* Image 2 - stays centered above the first image */}
                                <div className="absolute inset-0 flex items-center justify-center">
                                    <motion.img
                                        src={bannerImages[1]}
                                        alt="Hive Platform"
                                ...
                                {/* Image 3 - stacked slightly above image 2 and slides left on scroll */}
                                <motion.img
                                    src={bannerImages[2]}
                                    alt="Hive Platform Secondary"
                                    style={{
                                        opacity: opacity3,
                                        scale: scale3,
                                        y: y3,
                                    ...
                                    className="absolute inset-0 mx-auto max-w-full max-h-full w-auto h-auto object-contain drop-shadow-2xl will-change-transform origin-center"
                                />
                            </div>
                        </div>
                    </motion.div>
