  ...
  return (
    <DirectionContext.Provider value={{ direction, setAnimationDirection }}>
      <CurrentTabContext.Provider value={{ currentTab }}>
        <ProductsPageContext.Provider value={{ isProductsPage }}>
          <span
...
export const TriggerWrapper: React.FC<{ children: React.ReactNode }> = ({
  ...
  const { isProductsPage } = useContext(ProductsPageContext)!;
    ...
    <>
      {React.Children.map(children, (e, i) => (
        <button
          ...
          className={cn(
            "flex h-10 items-center gap-0.5 rounded-md px-4 py-2 text-sm font-medium transition-colors",
            isProductsPage
              ? "text-white hover:bg-white/10"
              : "text-neutral-950 dark:text-white",
            currentTab === i + 1
              ? isProductsPage
                ? "bg-white/20 [&>svg]:rotate-180"
  ...
  return (
    <>
      ...
      >
        ...
        <div
          className={cn(
            "rounded-md border backdrop-blur-xl transition-all duration-300",
            isProductsPage
